...
function findWasmBinary() {
  return locateFile('avrdude.wasm');
}
...
function avrdude_log(msg) { if (!window["avrdudeLog"]) { window["avrdudeLog"] = []; } window["avrdudeLog"] = [...window["avrdudeLog"], UTF8ToString(msg)]; }
function write_data(buf,len) { let writeAddress = window.writeAddressBuf[0]; let written = 0; while (written !== len) { const initialLength = Math.min(len - written, window.writeBuffer.length - writeAddress); const data = new Uint8Array(Module.HEAPU8.buffer.slice(buf + written, buf + written + initialLength)); window.writeBuffer.set(data, writeAddress); window.avrdudeLog = [...window.avrdudeLog, "Sending: " + Array.from(data).join(",")]; written += initialLength; writeAddress += initialLength; if (writeAddress === window.writeBuffer.length) { writeAddress = 3; }; }; window.writeAddressBuf[0] = writeAddress; }
function __asyncjs__clear_read_buffer(timeoutMs) { return Asyncify.handleAsync(async () => { window.avrdudeLog = [...window.avrdudeLog, "Clearing read buffer"]; window.avrDudeWorker.postMessage({ type: 'clear-read-buffer', timeout: timeoutMs }); await new Promise(resolve => { window.avrDudeWorker.onmessage = (event) => { if (event.data.type === "error") { window.funcs._errorCallback(); } resolve(); }; }); window.readAddress = 3; window.avrdudeLog = [...window.avrdudeLog, "Read buffer cleared"]; }); }
function __asyncjs__read_data(timeoutMs,length) { return Asyncify.handleAsync(async () => { const result = new Uint8Array(length); let read = 0; let end = Date.now() + timeoutMs; while (read !== length && end > Date.now()) { if (window.readAddress === window.readAddressBuf[0]) continue; let targetAddress = window.readAddressBuf[0]; if (targetAddress < window.readAddress) { targetAddress = window.readBuffer.length; }; targetAddress = Math.min(targetAddress, window.readAddress + length - read); result.set(window.readBuffer.slice(window.readAddress, targetAddress), read); read += targetAddress - window.readAddress; window.readAddress = targetAddress; if (window.readAddress === window.readBuffer.length) { window.readAddress = 3; } } if (read === 0) { window["avrdudeLog"] = [...window["avrdudeLog"], "Timeout"]; return; } const ptr = window.funcs._malloc(read * Uint8Array.BYTES_PER_ELEMENT); const data = result.slice(0, read); window.funcs.HEAPU8.set(result.slice(0, read), ptr); window["avrdudeLog"] = [...window["avrdudeLog"], "Received: " + Array.from(data).join(",")]; window.funcs._dataCallback(ptr, read); }); }
function __asyncjs__open_serial_port(baudRateInt) { return Asyncify.handleAsync(async () => { const serialOpts = { baudRate: baudRateInt, bufferSize: 1024*2 }; let port; if (!window.activePort) { port = await navigator.serial.requestPort(); await port.open(serialOpts); } else { port = window.activePort; if (!port.readable || !port.writable) { await port.open(serialOpts); } } await port.close(); await port.open({baudRate: 1200}); await port.setSignals({dataTerminalReady: false}); await new Promise(resolve => setTimeout(resolve, 100)); await port.setSignals({dataTerminalReady: true}); await port.close(); const worker = new Worker('avrdude-worker.js', { type: 'module' }); let portNumber = 0; if (navigator.serial) { const ports = await navigator.serial.getPorts(); for (let i = 0; i < ports.length; i++) { const port = ports[i]; if (port === window.activePort) { portNumber = i; break; } } } const writeBuffer = new SharedArrayBuffer(64 * 1024); const readBuffer = new SharedArrayBuffer(64 * 1024); worker.postMessage({ type: 'init', options: serialOpts, port: portNumber, writeBuffer, readBuffer, }); await new Promise(resolve => { worker.onmessage = (event) => { if (event.data.type === "error") { window.funcs._errorCallback(); } resolve(); }; }); window.writeBuffer = new Uint8Array(writeBuffer); window.readBuffer = new Uint8Array(readBuffer); window.writeAddressBuf = new Uint16Array(writeBuffer); window.readAddressBuf = new Uint16Array(readBuffer); window.writeAddressBuf[0] = 3; window.readAddressBuf[0] = 3; window.readAddress = 3; window.avrDudeWorker = worker; window.activePort = port; window["avrdudeLog"] = []; }); }
function __asyncjs__close_serial_port() { return Asyncify.handleAsync(async () => { window.avrDudeWorker.postMessage({ type: 'close' }); await new Promise(resolve => { window.avrDudeWorker.onmessage = (event) => { if (event.data.type === "error") { window.funcs._errorCallback(); } resolve(); }; }); window.activePort = null; window.avrDudeWorker.terminate(); }); }
function __asyncjs__is_serial_port_open() { return Asyncify.handleAsync(async () => { const port = window.activePort; return port.readable && port.writable; }); }
function __asyncjs__set_dts_rts(is_on) { return Asyncify.handleAsync(async () => { await window.activePort.setSignals({dataTerminalReady: is_on, requestToSend: is_on}); }); }
...
var _startAvrdude = Module['_startAvrdude'] = makeInvalidEarlyAccess('_startAvrdude');
var _malloc = Module['_malloc'] = makeInvalidEarlyAccess('_malloc');
var _dataCallback = Module['_dataCallback'] = makeInvalidEarlyAccess('_dataCallback');
var _errorCallback = Module['_errorCallback'] = makeInvalidEarlyAccess('_errorCallback');
...
function assignWasmExports(wasmExports) {
  ...
  assert(typeof wasmExports['startAvrdude'] != 'undefined', 'missing Wasm export: startAvrdude');
  assert(typeof wasmExports['malloc'] != 'undefined', 'missing Wasm export: malloc');
  assert(typeof wasmExports['dataCallback'] != 'undefined', 'missing Wasm export: dataCallback');
  assert(typeof wasmExports['errorCallback'] != 'undefined', 'missing Wasm export: errorCallback');
  ...
  _startAvrdude = Module['_startAvrdude'] = createExportWrapper('startAvrdude', wasmExports['startAvrdude'], 1);
  _malloc = Module['_malloc'] = createExportWrapper('malloc', wasmExports['malloc'], 1);
  _dataCallback = Module['_dataCallback'] = createExportWrapper('dataCallback', wasmExports['dataCallback'], 2);
  _errorCallback = Module['_errorCallback'] = createExportWrapper('errorCallback', wasmExports['errorCallback'], 0);
  ...
  dynCall_vidii = dynCalls['vidii'] = createExportWrapper('dynCall_vidii', wasmExports['dynCall_vidii'], 5);
...
var wasmImports = {
  ...
  /** @export */
  __asyncjs__clear_read_buffer,
  /** @export */
  __asyncjs__close_serial_port,
  /** @export */
  __asyncjs__open_serial_port,
  /** @export */
  __asyncjs__read_data,
  /** @export */
